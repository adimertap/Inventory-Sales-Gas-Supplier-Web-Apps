...
<main>
                ...
                <div class="row align-items-center justify-content-between pt-3">
                    <div class="col-auto mb-3">
                        <h1 class="page-header-title">
                            ...
                            Edit Data Penjualan {{ $item->kode_Penjualan }}
                        </h1>
                    </div>
                </div>
            </div>
    ...
    <div class="container-fluid mt-4">
        <form action="{{ route('penjualan.update', $item->id_penjualan) }}" id="form" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="row">
                        ...
                        <div class="card-body">
                            <div class="row justify-content-center">
                                <div class="col-xxl-12 col-xl-8">
                                    <div class="form-row">
                                        <input type="hidden" name="grand_total_keseluruhan" value="{{ $item->grand_total }}">
                                        ...
                                        <div class="form-group col-md-4">
                                            ...
                                            <input class="form-control" id="kode_penjualan" type="text" name="kode_penjualan"
                                                value="{{ $item->kode_penjualan }}" readonly />
                                        </div>
                                    ...
                                    <div class="form-row">
                                        <div class="form-group col-md-3">
                                            ...
                                            <select class="form-control" name="status_bayar" class="form-control" id="status_bayar">
                                                <option value="{{ $item->status_bayar }}">{{ $item->status_bayar }}</option>
                                        ...
                                        <div class="form-group col-md-5">
                                            ...
                                            <div class="input-group input-group-joined">
                                                ...
                                                <select class="form-control" name="id_customer" class="form-control"
                                                    id="id_customer">
                                                    <option value="{{ $item->Customer->id_customer }}">{{ $item->Customer->nama_customer }}</option>
                                                    @foreach ($customer as $tes)
                                                    <option value="{{ $tes->id_customer }}">{{ $tes->nama_customer }}
                                                    </option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        ...
                                        <div class="form-group col-md-4">
                                            ...
                                            <select class="form-control" name="id_produk" class="form-control"
                                                ...
                                                <option value="{{ $produks->id_produk }}" id="{{ $produks->stok }}">{{ $produks->nama_produk }},
                ...
                <div class="table-responsive table-billing-history">
                    <table class="table mb-0" id="table_detail">
                        ...
                        <tbody id="konfirmasi">
                            ...
                            <tr id="item-{{ $tes->id_detail_penjualan }}" role="row" class="odd">
                                <td><span id="{{ $tes->id_produk }}">{{ $tes->nama_produk }}, {{ $tes->Kategori->nama_kategori }}</span></td>
                                <td><span id="{{ $tes->pivot->harga_jual }}">{{ $tes->pivot->jumlah_jual }}/{{ number_format($tes->pivot->harga_jual) }}</span></td>
                                <td><span id="{{ $tes->pivot->jumlah_jual }}">{{ number_format($tes->pivot->total_jual) }}</span></td>
                                <td>
...
<script>
    ...
    $('#id_produk').change(function () {
        ...
        var stok = $(this).find("option:selected").attr('id')
        ...
        for (let index = 0; index < data.length; index++) {
            ...
            // Id Produk
            var id_produk_tes = children[0]
            var span_produk = $(id_produk_tes).children()[0]
            var id_produk = $(span_produk).attr('id')
            ...
            if(id_produk == id){
            $('#harga_produk_modal').val(harga_jual)
            $('#jumlah_produk_modal').val(jumlah_jual)
            }else{
                $('#harga_produk_modal').val()
                $('#jumlah_produk_modal').val()
            }
        }
